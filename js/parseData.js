function getImages(dados) {
  ...
  for (let index = 0; index < coisasPersonagens.length; index++) {
    ...
    quadrado.querySelector("#i" + index + "").src =
      primeiro["thumbnail"]["path"] + "." + primeiro["thumbnail"]["extension"];
    quadrado.querySelector("#n" + index + "").textContent =
      "Nome: " + primeiro["name"];
    quadrado.querySelector("#cod" + index + "").textContent =
      "Id: " + primeiro["id"];
  }
}
